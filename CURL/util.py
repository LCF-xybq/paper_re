...
class ImageProcessing(object):
    ...
    def rgb_to_lab(img, is_training=True):
        ...
        img = torch.mul(img, torch.Tensor(
            [1 / 0.950456, 1.0, 1 / 1.088754]).cuda())
        ...
        fxfyfz_to_lab = torch.Tensor([[0.0, 500.0, 0.0],
                                      [116.0, -500.0, 200.0],
                                      [0.0, 0.0, -200.0]]).cuda()

        img = torch.matmul(img, fxfyfz_to_lab) + torch.Tensor([-16.0, 0.0, 0.0]).cuda()
    ...
    def lab_to_rgb(img, is_training=True):
        ...
        lab_to_fxfyfz = torch.Tensor([  # X Y Z
        ...
        img = torch.matmul(
            img + torch.Tensor([16.0, 0.0, 0.0]).cuda(), lab_to_fxfyfz)
        ...
        img = torch.mul(img,
            torch.Tensor([0.950456, 1.0, 1.088754]).cuda())

        xyz_to_rgb = torch.Tensor([  # X Y Z
    ...
    def load_image(img_filepath, normaliser):
        img = ImageProcessing.normalise_image(np.array(Image.open(img_filepath)),
    ...
    def rgb_to_hsv(img):
        ...
        zero = torch.Tensor(torch.zeros(shape[0:2])).cuda()
    ...
    def apply_curve(img, C, slope_sqr_diff, channel_in, channel_out,
                    clamp=False):
        slope = torch.zeros((C.shape[0] - 1)).cuda()
    ...
    def adjust_hsv(img, S):
        ...
        slope_sqr_diff = (torch.zeros(1) * 0.0).cuda()
